//cpp
// NONMATCHING: floor(ordering) div=1 (+4 bytes). Body otherwise byte-identical.
// Residual: operator_new(0x14) buffer at +0x588 zero-fill.
// ROM: mov r1,#5; mov r0,#0; 4x strb r0,[r2],#1; subs; bne (CLEAR expander shape).
// Pattern is unique in the entire ROM. Under mwccarm 1.2/sp2p3 -O4,p:
//   - only stack u8[0x14]={0} emits that post-inc form
//   - hand-unroll always folds to strb[rN,#0..3]+add#4 (all versions/opt levels)
//   - for(i=0;i<0x14;i++)*q++=0 never unrolls; __clear / new u8[n]() stay as bl
//   - compound/ctor assign = stack CLEAR + copy to heap
// Field +0x588 is int[5]/pointer slots (see func_ov002_020d6334 / 020bdc18).
