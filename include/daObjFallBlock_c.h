 ...
 *   0x330  unique ID of a linked actor 0xb2. On actorID 0x53 (FALL_BLOCK_LLL)
 *          with mSuppressed set, Behavior calls func_ov098_0213a0e8 to fill it
 *          (when 0) or func_ov098_0213a0a8 to poll it (when set).
 *   0x334  the shake step added to mAngleX each frame in case 2.
 *   0x336  the same for mAngleZ (0x90), not mAngleY.
 *   0x338  shake-phase, walked through the sine table in case 1 to bob Y.
 ...
 *   0x344  pointer to a same-id neighbour behind this block (angle diff
 *          >= 0x4000). Zero means this block is the head of the forward chain.
